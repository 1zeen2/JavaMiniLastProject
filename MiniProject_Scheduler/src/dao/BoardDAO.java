...
public class BoardDAO {
	...
	/*
	 * connection prepardStatement ResultSet
	 * 
	 * ResultSet.close() PrepardStatement.close() Connection.close() 열기와 역순으로 닫아준다
	 */
	// 오라클 닫기
	...
	/*
	 * Collection Collection ______|__________________ | | | List set Map ==>
	 * interface => Map map = new => Set set = new Hash Set => List list = new
	 * ArrayList() | | | ArrayList HashSet HashMap ---------- -------- --------- =>
	 * 클래스 관리 / SQL 문장 관리 => 키, 값 => 두개를 동시에 저장 키(중복이 없다), 값(중복이 가능) => Cookie /
	 * Session => 웹 실시간 채팅 (사용자 정보) 순서는 없다 데이터중복 허용하지 않음 => 데이터베이스에서 데이터를 모아서 저장 순서가
	 * 존재 (인덱스) 데이터 중복을 허용 비동기 방식 => ORDER BY 를 사용하지 않으면 순서가 일정하지 않는다
	 * 
	 */
...
//								 -------------------- 자동증가 번호
			...
			ps.executeUpdate();
			/*
			 * executeQuery() => 결과값이 있다 => SELSECT executeUpdate() => 결과값이 없다 => commit()
