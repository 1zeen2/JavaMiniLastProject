...
public class CalendarDAO {
	...
	public List<CalendarVO> calendarListData(int page) {
		...
		try {
			...
			String sql = "SELECT title, place, day, content, userId, num , no "
					...
					+ "FROM (SELECT title, place, day, content, userId , no " + "FROM schedule ORDER BY no DESC))"
	...
	public void calendarInsert(CalendarVO vo) {
		...
		try {
			...
			getConnection();
			/*
			 * TITLE VARCHAR2(4000) PLACE VARCHAR2(4000) DAY DATE CONTENT VARCHAR2(4000)
	...
	public CalendarVO calendarDetailData(int no) {
		...
		try {
			getConnection();
			/*
			 * String sql = "UPDATE calendar SET " +"WHERE no = ?"; ps =
			 * conn.prepareStatement(sql); ps.setInt(1, no); ps.executeUpdate();
			 */
			// 데이터 읽기
			String sql = "SELECT title, place, day, content, userId , no" + "FROM schedule " + "WHERE no=? ";
	...
	public CalendarVO calendarUpdateData(int no) {
		...
		try {
			getConnection();
			String sql = "SELECT title, place, day, content, userId " + "FROM schedule " + "WHERE no=? ";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, no);
			ResultSet rs = ps.executeQuery();
			rs.next();
	...
	public void CalendarUpdate(CalendarVO vo) {
		...
		try {
			getConnection();
			String sql = "UPDATE schedule SET " + "title =?, place=?, day=?, content=? " + "WHERE no=? ";
			ps = conn.prepareStatement(sql);
			ps.setString(1, vo.getTitle());
			ps.setString(2, vo.getPlace());
			ps.setString(3, vo.getDay());
