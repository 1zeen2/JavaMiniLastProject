...
public class 문자열배열_3 {
	...
	public static void main(String[] args) {
		...
		for (String f : 과일) {
			if(f.contains(fd)) {	//	startsWith도 사용이 가능하다
				System.out.println("영어는 " + f.replaceAll("[^A-Za-z]", ""));
				/*
				 * 	정규식 => 문자열에 패턴을 만들어주는 방식이다.
		...
		for (int i = 0; i < 과일.length; i++) {
			영어과일명[i] = 과일[i].substring(과일[i].indexOf(":") + 1);		// 필요한 문자 만큼 자르는 경우 substring
		}																	// 원하는 문자의 위치 => indexOf
		//	자른 값을 출력 문자열 형으로 출력
		for (String ef:영어과일명) {			
			System.out.println(ef);											// String => 문자를 쉽게 제어할 수 있게 만든 클래스
		}
		int max = 0;
		int min = 100;
		for (String ef : 영어과일명) {
			if (max < ef.length())											//	문자열 개수가 몇 개인지
				max = ef.length();											
			if (min > ef.length())											//	배열의 개수를 셀 때 	==> length
				min = ef.length();											//	문자열의 개수를 셀 때	==> length()
		}
		for (String ef : 영어과일명) {
			if (ef.length() == max) {
