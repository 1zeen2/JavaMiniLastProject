 ...
 *						저장	=====> 이름을 부여	=> 변수
 *				12번지-------------
 *				** 식별자	=>	한 개의 클래스 내에서는 같은 변수 명을 사용할 수 없다.
 *					= 알파벳이나 한글로 시작한다.
 *						(알파벳은 대소문자를 구분한다.)
 *					= 문자의 갯수는 제한이 없다. ( 3 ~ 10자로 압축 )	=> 어떤 데이터가 저장되어 있는지 확인하기 위해.
 *					= 숫자를 사용할 수 있다. ( 앞에 사용 금지 )
 *					= 특수문자를 사용할 수 있다. ( _, $ )	=>	_( 위치는 상관 없다. )
 *					= 키워드	: 자바에서 사용하는 문자열(단어)
 *						int / double .... ( 빨간색이 나오면 단어를 바꿔줘야 한다. )
 *							int int => 오류 발생
 *					=공백을 사용할 수 없다.
 *						int a b => 오류 발생
 *			2.	메모리 크기 (데이터형)
 ...
 *							=>	byte는 연산 처리가 되면 결과 값 int
 *								--------------------------
 *								byte + byte = int
 *								byte a = 127, b = 127
 *								a + b = int ==> 사용 범위 초과를 해결하기 위해
 *					int	( 4byte )	=>	프로그램에서 가장 많이 사용하는 데이터형
 *											정수 => int => default
 ...
 *										----- double로 인식	
 *							float f = 10.5F(f)
 *					double	( 8byte )	=>	실수 중 가장 많이 사용되는 데이터형
 *						=>	default	( 컴퓨터에서 실수가 있는 경우	=>	double )
 *						=>	실수 점 15자리 수 까지 인식
 *
 *					**	정수	<	실수	=>	큰 숫자에 대한 표현 ( byte 크기 아님)
 *						long<	float 
 *				------------------------------------------
 *					문자	:	char ( 2byte ) => 문자는 음수가 없다. ( 0 ~ 65535)
 *							=> 각 국의 언어를 표현	=>	유니코드	=>	호환성이 좋다.
 *							값은	=>	문자 '', 정수 값 사용이 가능
 *						형식)	
 *							char a = 'A'
 *							char b = 65 ====== 예외 사항
 *						=>	char는 연산 처리 시에 정수형으로 바뀜
 *							'A'	=>	65
 *							'a'	=>	97
 *							'0'	=>	48
 *				------------------------------------------
 ...
 *						형삭) boolean	b =	true	/	boolean	b = true
 *							** 크기는 비교할 수 없다.
 *							** 기본 연산이 안된다.
 *								--------	부정 연산자, 비교 연산자
 *											논리 연산자, 삼항 연산자
 *						------------------------------------------
 *					참조	( 배열, 클래스 )	=> 메모리 주소를 이용.
 *				------------------------------------------
 *			3.	변수 형식
 *					데이터형 변수명;
 *					***사용시에는 반드시 변수 명 = 값을 대입해야 사용 가능.
 *					정수
 *						byte b	= 10;
 *						int	 a	= 10;
 *						long l	= 100L;
 *			4.	초기 값
 *					임의로 지정	=> 변수 ( 필요 시에 따라 변경이 가능 )
 *		 			1)	선언	=>	초기 값
 ...
 *			= 산술연산자
 *				+, -, *, /, % ( 나머지 )
 *			= 비교연산자	:	결과 값이 boolean (true / false)
 *				==(같다)	6==7 (false)
 *				!=(같지 않다) 6!=7 *(true)
 *				기준점 : 왼쪽
 *				<	(작다)
 ...
 *				&& => 직렬연산자 (범위, 기간 안에 있는 경우) => 예약, 체크인/아웃
 *				\\ => 병렬연산자 (범위나 기간을 벗어나는 경우)
 *			= 대입연산자 : 결과 값을 다시 저장할 경우
 *				= 대입
 *				int a = 10
 *				+=
 *				-=
 ...
 *		int + long = long
 *		---
 *		 l long 변경 후에 연산 ===> 데이터형을 큰형으로 변경 후에 연산		
 *		char + int = int
 *		----
 *		 l int
 *		*** 데이터형의 크기
 ...
 *			20.5		65.0
 *		---------------------
 *			85.5	=>	결과 값은 가장 큰 데이터형이 나온다.
 *		=> +, -, *	=> 동일
 *
 *		/
 *		=>	0으로 나누면 오류 발생 10/0 => 오류 => 10을 0으로 나눌 수 없다.
