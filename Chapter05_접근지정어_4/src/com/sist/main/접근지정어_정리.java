 ...
 *                com.회사명.manager		=> 파일, 이메일, 뉴스(외부에서API를 이용할 때)
 *                com.회사명.vo 			=> 데이터형 (사용자 정의)
 *                com.회사명.service		=> BI (통합)=> 조립
 *                com.회사명.controller	=> 브라우저와 연결할 때 사용
 *        => 패키지는 반드시 첫번째 줄에 사용한다 => 한 번만 사용이 가능
 *        => 패키지를 불러오는 방법
 ...
 *         
 *         Date date = new Date(); =>  오류발생	==> Date 가 두 개 이기 때문에 어떤 것을 가져와야 하는지 인식을 하지 못함
 *    -------------------------------------------------------
 *      변수    연산자   제어문
 *             ---------
 *                 |
