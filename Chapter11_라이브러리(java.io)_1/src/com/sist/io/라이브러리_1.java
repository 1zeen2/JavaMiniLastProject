 ...
 *           	File 생성자
 *           	-----------------------------------
 *           		File file = new File("경로 명 / 파일 명") => 파일에 대한 정보를 가지고 있다.
 *           		File dir = new File("경로명") => 폴더에 대한 정보를 가지고 있다.
 *            	File 정보와 관련된 메서드
// *           	*** 1) 파일 명 읽기					=> getName()		==> String
 *           		2) 경로 명 읽기					=> getParent()		==> String
 *           	*** 3) 파일 명 + 경로 명				=> getPath()		==> String
 *           	*** 4) 파일 크기						=> length()			==> long
 *           		5) 숨긴 파일 여부				=> isHidden()		==> boolean
 *           		6) 읽기 가능 여부				=> canRead()		==> boolean
 *           		7) 쓰기 가능 여부				=> canWrite()		==> boolean
 *           		8) 수정일						=> lastModified()	==> long
 *           	*** 9) 파일 정보를 한 번에 읽기(폴더)	=> listFiles()		==> listFiles():File[]
