// 학생 3명의 국, 영, 수 점수 => 평균을 내고 학점을 받아라.
...
public class 자바제어문_반복문_9 {
	...
	public static void main(String[] args) {
		...
		/* 클래스를 메모리에 저장할 수 있는 새로운 공간을 만든다.
		 * 지역 변수는 메모리에서 관리하고 블록이 끝나면 자동으로 메모리에서 삭제한다.
		 * new는 프로그래머가 관리할 수 있게 해주며 자동으로 사라지지 않고 메모리에 계속 존재한다.
		 * 메모리 누수 현상 ==> GC(가비지 컬렉션 => 프로그램 종료 후 회수)
		 */
		for (int i = 0; i < 3; i++) {
